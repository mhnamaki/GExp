...
public class BidirectionalKWS {

	GraphInfraReaderArray graph;
	// private static String graphInfraPath =
	// "/Users/zhangxin/Desktop/AQPEQ/GraphExamples/k4Infra/";
	// private static String keywordsPath =
	// "/Users/zhangxin/Desktop/AQPEQ/KeywordExamples/JamesJohnDatabase.in";
	...
	public BidirectionalKWS() {

	}

	public static void main(String[] args) throws Exception {
		BidirectionalKWS bidirectionalKWS = new BidirectionalKWS();
		bidirectionalKWS.run(bidirectionalKWS);
	}

	private void run(BidirectionalKWS bidirectionalKWS) throws Exception {
	...
	private void BidirExpSearch() {
		...
		while (!qIn.isEmpty() || !qOut.isEmpty()) {
			...
			System.out.println("new iteration cnt: " + cnt);
			if (!qIn.isEmpty()) {
			...
			if (nodeFromQIn == null) {
				...
				if (v.depth < dMax) {
					...
					if (qIn.isEmpty()) {
						System.out.println("qIn is empty after checking incoming of v=" + v);
					} else {
						System.out
								.println("qIn head -> " + qIn.peek().nodeId + " activation :" + qIn.peek().activation);
					}
	...
	private double computeNewActivation(Node v, Node u, String keyword) {
		...
		// spreadActivation: true -> incoming outgoing -> outgoing
		if (spreadActivation) {
			// spread to incoming
