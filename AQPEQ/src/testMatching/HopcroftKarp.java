 ...
 * <em>minimum (cardinality) vertex cover</em> in a bipartite graph. A
 * <em>bipartite graph</em> in a graph whose vertices can be partitioned into
 * two disjoint sets such that every edge has one endpoint in either set. A
 * <em>matching</em> in a graph is a subset of its edges with no common
 * vertices. A <em>maximum matching</em> is a matching with the maximum number
 * of edges. A <em>perfect matching</em> is a matching which matches all
 * vertices in the graph. A <em>vertex cover</em> in a graph is a subset of its
 * vertices such that every edge is incident to at least one vertex. A
 * <em>minimum vertex cover</em> is a vertex cover with the minimum number of
 * vertices. By Konig's theorem, in any biparite graph, the maximum number of
 * edges in matching equals the minimum number of vertices in a vertex cover.
 * The maximum matching problem in <em>nonbipartite</em> graphs is also
 ...
 * <p>
 * See also {@link BipartiteMatching}, which solves the problem in O(
 * <em>E V</em>) time using the <em>alternating path algorithm</em> and
 * <a href =
 * "https://algs4.cs.princeton.edu/65reductions/BipartiteMatchingToMaxflow.java.html">
 * BipartiteMatchingToMaxflow</a>, which solves the problem in O(<em>E V</em>)
 * time via a reduction to the maxflow problem.
 * <p>
...
public class HopcroftKarp {
	...
	public HopcroftKarp(Graph G) {
		...
		for (int o = 0; o < G.V(); o++) {
			System.out.println(o + ":" + bipartition.color(o));
		}
