...
public class PairwiseKeywordSearchDemo {
    ...
    private String distanceIndexDB = "/Users/mnamaki/Documents/Education/PhD/Summer2017/AQEQ/Datasets/dbp/dbpGraphInfra/distanceIndex/dbp_8bits.jin";
    ...
    private static double[] deltas = {10};// quality bound
    ...
    public PrunedLandmarkLabeling prunedLandmarkLabeling;
    ...
    // from bank
    public static HashMap<String, HashSet<Integer>> candidatesOfAKeyword = new HashMap<String, HashSet<Integer>>();
    public static HashSet<Integer> candidatesSet = new HashSet<Integer>();
    public static HashMap<Integer, HashSet<Integer>> nodeIdsOfToken = new HashMap<Integer, HashSet<Integer>>();
    ...
    boolean keywordSuggestionOn = false;
    ...
    IncEval incEvalNoIndex = null;
    public IncEval incEvalWithIndex = null;

    public static void main(String[] args) throws Exception {

        PairwiseKeywordSearchDemo kws = new PairwiseKeywordSearchDemo();
        kws.runMain(args);
                    ...
                    for (double delta : deltas) {
                        for (int maxRequiredResult : maxRequiredResults) {
                            ...
                            while (exp < numberOfSameExperiments) {
                                ...
                                double incEvalNoIndexTotalCostPlusInitCost = 0d;
                                double incEvalWithIndexTotalCostPlusInitCost = 0d;
                                ...
                                if (keywordSuggestionOn) {
                                    ...
                                    // incEvalNoIndex = new IncEval(graph,
                                    // graphKWSExpand.bestKeywordInfo,
                                    // topNAnswers,
                                    // distanceBound,
                                    // DummyProperties.KWSSetting.SUBGRAPH);

                                    incEvalWithIndex = new IncEval(graph, graphKWSExpand.bestKeywordInfo, topNAnswers,
                                            distanceBound, DummyProperties.KWSSetting.SUBGRAPH);

                                    // if (incEvalOn) {
                                    //
                                    // System.out.println("starting our inc
                                    // eval" + new DateTime());
                                    //
                                    // incEvalNoIndex.incEval();
                                    //
                                    // for (int p = 0; p < topNAnswers.size();
                                    // p++) {
                                    // incEvalNoIndexTotalCostPlusInitCost +=
                                    // topNAnswers.get(p).getCost()
                                    // +
                                    // incEvalNoIndex.lastTripleOfKeywordMatchToTarget[p].getCost();
                                    // }
                                    //
                                    // System.out.println("after incEval: " +
                                    // incEvalNoIndex.incEvalDuration);
                                    ...
                                    if (incEvalOn) {
                                        ...
                                        System.out.println("starting our inc eval with index" + new DateTime());

                                        incEvalWithIndex.incEval(prunedLandmarkLabeling);

                                        for (int p = 0; p < topNAnswers.size(); p++) {
                                            incEvalWithIndexTotalCostPlusInitCost += topNAnswers.get(p).getCost()
                                                    + incEvalWithIndex.lastTripleOfKeywordMatchToTarget[p].getCost();
                                        }

                                        System.out.println(
                                                "after incEval with index: " + incEvalWithIndex.incEvalDuration);
                                        System.out.println(
                                                "incEval cost with index: " + incEvalWithIndexTotalCostPlusInitCost);
                                    }
                                    ...
                                    if (coOccBaseLineOn) {
                                        ...
                                        if (topNAnswersTemp.size() > this.fixedmaxRequiredResult) {
                                            topNAnswers.addAll(topNAnswersTemp.subList(0, this.fixedmaxRequiredResult));
                                        } else {
                                            topNAnswers.addAll(topNAnswersTemp);
                                        }
                                        ...
                                        if (fullOutputsQueue.size() < this.fixedmaxRequiredResult) {
                                            double addedValue = (double) graphKWSExpand.initialOveralWeight
                                                    / (double) this.fixedmaxRequiredResult;
                                            int lessResult = this.fixedmaxRequiredResult - fullOutputsQueue.size();
                                ...
                                timeInfos.add(new InfoHolder(23, "incEval No index Duration",
                                        incEvalNoIndex != null ? incEvalNoIndex.incEvalDuration : ""));
                                timeInfos.add(new InfoHolder(24, "incEval No index visited nodes",
                                        incEvalNoIndex != null ? incEvalNoIndex.visitedNodes : ""));
                                timeInfos.add(new InfoHolder(25, "incEval No indextotal cost",
                                        incEvalOn ? incEvalNoIndexTotalCostPlusInitCost : ""));
                                ...
                                timeInfos.add(new InfoHolder(44, "incEval With Index Duration",
                                        incEvalWithIndex != null ? incEvalWithIndex.incEvalDuration : ""));
                                timeInfos.add(new InfoHolder(45, "incEval With Index visited nodes",
                                        incEvalWithIndex != null ? incEvalWithIndex.visitedNodes : ""));
                                timeInfos.add(new InfoHolder(46, "incEval With Index total cost",
                                        incEvalOn ? incEvalWithIndexTotalCostPlusInitCost : ""));

                                timeInfos.add(new InfoHolder(47, "10 sugg kywrds", sugg));
    ...
    public LinkedList<ResultTree> runFromOutside(ArrayList<String> keywords, HashMap<String, HashSet<Integer>> candidatesOfAKeyword,
                                                 int distanceBound, int maxRequiredResult, GraphInfraReaderArray graph, boolean debugMode)
            throws Exception {
        // candidatesSet
        ...
        LinkedList<ResultTree> resultTrees = new LinkedList<ResultTree>();

        while (!fullOutputsQueue.isEmpty()) {
            PairwiseAnswer pAnswer = fullOutputsQueue.poll();

            ResultTree currentResTree = new ResultTree();
            ListenableUndirectedGraph<ResultNode, RelationshipInfra> anOutput = new ListenableUndirectedGraph<ResultNode, RelationshipInfra>(
                    RelationshipInfra.class);
            currentResTree.cost = (double) pAnswer.pairwiseWeight;
            HashMap<ResultNode, Integer> nodeIdOfResNode = new HashMap<ResultNode, Integer>();
            HashMap<Integer, ResultNode> resNodeOfNodeId = new HashMap<Integer, ResultNode>();

            for (int nodeId : pAnswer.nodeMatches) {
                String labels = "";
                for (int tokenId : graph.nodeOfNodeId.get(nodeId).getTokens()) {
                    labels += StringPoolUtility.getStringOfId(tokenId).toLowerCase() + ", ";
                }
                ResultNode resNode = new ResultNode(labels, nodeId, graph.nodeOfNodeId.get(nodeId));
                anOutput.addVertex(resNode);
                nodeIdOfResNode.put(resNode, nodeId);
                resNodeOfNodeId.put(nodeId, resNode);
            }

            graph.tempMaxRelIdForDemo = graph.maxRelId;

            for (ResultNode resNode1 : anOutput.vertexSet()) {
                for (ResultNode resNode2 : anOutput.vertexSet()) {
                    if (resNode1.nodeId < resNode2.nodeId) {
                        RelationshipInfra relInfra = new RelationshipInfra(++graph.tempMaxRelIdForDemo, resNode1.nodeId, resNode2.nodeId);
                        int distance = prunedLandmarkLabeling.queryDistance(resNode1.nodeId, resNode2.nodeId);
                        int tokenId = StringPoolUtility.insertIntoStringPool(String.valueOf(distance));
                        relInfra.addType(tokenId);
                        anOutput.addEdge(resNodeOfNodeId.get(resNode1.nodeId), resNodeOfNodeId.get(resNode2.nodeId), relInfra);
                    }
                }
            }


            currentResTree.anOutputTree = anOutput;
            resultTrees.add(currentResTree);
        }
    ...
    private void run(ArrayList<String> keywords) throws Exception {
        ...
        //updatePairwiseAnswersWithSteinerResultTree(keywords, fullOutputsQueue);
    ...
    public ArrayList<AnswerAsInput> tansformResultTreeIntoAnswerAsInput(LinkedList<ResultTree> fullOutputsQueue) {
        ...
        while (!fullOutputsQueue.isEmpty()) {
            ...
            AnswerAsInput topNAnswer = new AnswerAsInput(-1, contentNodes, allNodes, cost);
            topNAnswers.add(topNAnswer);
        }
        while (!tem.isEmpty()) {
    ...
    public HashMap<Integer, CostAndNodesOfAnswersPair> keywordExpand(ArrayList<AnswerAsInput> topNAnswers,
        ...
        graphKWSExpand = new GraphKWSExpand(graph, topNAnswers, cost, exploreRange, keywordsSet);
