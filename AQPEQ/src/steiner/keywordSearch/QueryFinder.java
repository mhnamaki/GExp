...
public class QueryFinder {
	...
	private static String keywordsPath = "/Users/mnamaki/Documents/Education/PhD/Summer2017/AQEQ/Datasets/imdb/queries/temp/imdb_query_DR.txt";
	...
	private static String newKeywordPath = "/Users/zhangxin/Desktop/IMDB/keyword/imdb_query_DR.txt";

	private static int[] maxRequiredResults = { 2 };
	private static int[] distanceBounds = { 3 };
	private static double[] deltas = { 1.0 };//// quality bound
	...
	public static void main(String[] args) throws Exception {
		...
		QueryFinder stinerbasedKWS = new QueryFinder();
		ArrayList<QueryObject> querieslist = stinerbasedKWS.runOptimalSteinerExperiment();
		stinerbasedKWS.writeQuery(querieslist);

	}

	private ArrayList<QueryObject> runOptimalSteinerExperiment() throws Exception {
		...
		ArrayList<QueryObject> queriesList = new ArrayList<QueryObject>();
		int queryNum = 0;
					...
					for (double delta : deltas) {
						for (int maxRequiredResult : maxRequiredResults) {
							...
							while (exp < numberOfSameExperiments) {
								...
								if (initialKWSDuration < maxTimeBound) {

									if (isValidQuery) {
										// it's a valid query
										// writeIntoFile
										ArrayList<String> tokens = new ArrayList<String>();
										for (String keryword : keywords) {
											for (String token : DummyFunctions.getTokens(keryword)) {
												tokens.add(token);
											}
										}
										QueryObject query = new QueryObject(distanceBound, keywords.size(), tokens);
										queriesList.add(query);
										System.out.println("found query -> " + queryNum++);
									}

//									if (queriesList.size() == 20) {
//										return queriesList;
//									}
								}
	...
	public HashMap<Integer, CostAndNodesOfAnswersPair> keywordExpand(ArrayList<AnswerAsInput> topNAnswers,
		...
		steinerKWSExpand = new SteinerKWSExpansion(graph, topNAnswers, delta, distanceBound, berkeleyDB, keywordsSet);
	...
	public void writeQuery(ArrayList<QueryObject> querieslist) throws Exception {
		System.out.println("writing query");
		File queryF = new File(newKeywordPath);
		FileOutputStream fosQ = new FileOutputStream(queryF);
		BufferedWriter bw = new BufferedWriter(new OutputStreamWriter(fosQ));
		for (QueryObject query : querieslist) {
			int b = query.distanceBound;
			int l = query.l;
			bw.write("b:" + b + ",l:" + l + ";");
			int i = 0;
...
class QueryObject {
	...
	QueryObject(int distanceBound, int l, ArrayList<String> tokens) {
		this.distanceBound = distanceBound;
		this.l = l;
		this.tokens = tokens;
	}
