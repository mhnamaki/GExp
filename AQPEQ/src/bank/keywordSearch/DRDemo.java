...
public class DRDemo {
	...
	// xin
	public static String graphInfraPath = "/Users/zhangxin/AQPEQ/GraphExamples/demo/";
	private static String keywordsPath = "/Users/zhangxin/AQPEQ/KeywordExamples/AC.in";
	private static String envFilePath = "/Users/zhangxin/Desktop/IMDB/sample/dbEnvNoProp";

	// // mhn
//	private static String graphInfraPath = "/Users/mnamaki/AQPEQ/GraphExamples/k1Infra/graph/";
//	private static String keywordsPath = "/Users/mnamaki/AQPEQ/KeywordExamples/AC.in";
//	private static String envFilePath = "/Users/zhangxin/Desktop/IMDB/sample/dbEnvNoProp";
	public IncEval incEval;
	public int maxRequiredResult=0;
	public int distanceBound=0;
	public double delta=0.0;//// quality bound
	public ArrayList<String> keywords;
	public HashMap<Integer, CostAndNodesOfAnswersPair> estimatedWeightOfSuggestedKeywordMap = null;

	int heapSize = 0;

	public HashMap<String, HashSet<Integer>> candidatesOfAKeyword = new HashMap<String, HashSet<Integer>>();
	public HashSet<Integer> candidatesSet = new HashSet<Integer>();
	public HashMap<Integer, HashSet<Integer>> nodeIdsOfToken = new HashMap<Integer, HashSet<Integer>>();
	...
	public RootKWSExpand rootKWSExpand;
	...
	static boolean keywordSuggestionOn = false;
	...
	public static void main(String[] args) throws Exception {
		...
		DRDemo experimentUsingBDB = new DRDemo();
		experimentUsingBDB.runBankExperiment();

	}

	public LinkedList<ResultTree> runBankExperiment() throws Exception {
		...
		while (exp < numberOfSameExperiments) {
			...
			// AnswerAsInput
			ArrayList<AnswerAsInput> topNAnswersTemp = tansformResultTreeIntoAnswerAsInput(this.fullOutputsQueue);
			...
			if (keywordSuggestionOn) {
				...
				if (qualityExp) {
					...
					estimatedWeightOfSuggestedKeywordMap = keywordExpand(topNAnswers, keywords, delta, distanceBound);

					if (estimatedWeightOfSuggestedKeywordMap == null || estimatedWeightOfSuggestedKeywordMap.size() == 0
							|| rootKWSExpand.bestKeywordInfo == null || rootKWSExpand.bestKeywordInfo.nodeId == null) {
					...
					for (int tokenId : estimatedWeightOfSuggestedKeywordMap.keySet()) {
						avgSuggestedWeightSecondDelta += estimatedWeightOfSuggestedKeywordMap.get(tokenId).cost;

						minSuggestedWeightSecondDelta = Math.min(estimatedWeightOfSuggestedKeywordMap.get(tokenId).cost,
								minSuggestedWeightSecondDelta);

						maxSuggestedWeightSecondDelta = Math.max(estimatedWeightOfSuggestedKeywordMap.get(tokenId).cost,
				...
				for (int tokenId : estimatedWeightOfSuggestedKeywordMap.keySet()) {
					avgSuggestedWeightMainDelta += estimatedWeightOfSuggestedKeywordMap.get(tokenId).cost;

					minSuggestedWeightMainDelta = Math.min(estimatedWeightOfSuggestedKeywordMap.get(tokenId).cost,
							minSuggestedWeightMainDelta);

					maxSuggestedWeightMainDelta = Math.max(estimatedWeightOfSuggestedKeywordMap.get(tokenId).cost,
				...
				System.out.println("keyword suggestion visited nodes " + rootKWSExpand.visitedNodes);
				System.out.println("keyword suggestion visited keywords " + rootKWSExpand.visitedKeywords);

				// incremental evaluation
				incEval = new IncEval(graph, rootKWSExpand.bestKeywordInfo, topNAnswers, distanceBound,
				...
				if (incEvalOn) {
					...
					for (int p = 0; p < topNAnswers.size(); p++) {
						...
						System.out.println("inc eval p:" + p + ", nodeId: "
					...
					System.out.println(
							"selected keyword nodeIds were: " + Arrays.toString(rootKWSExpand.bestKeywordInfo.nodeId));
					System.out.println("lowest weight suggested keywrd: "
				...
				if (newKWSOn) {
					...
					newKeywords.add(StringPoolUtility.getStringOfId(rootKWSExpand.lowestWeightSuggestedKeywordId));
					...
					if (fullOutputsQueue.size() < maxRequiredResult) {
						double addedValue = (double) rootKWSExpand.initialOveralWeight / (double) maxRequiredResult;
					...
					if (debugMode && visualizeMode) {
						LinkedList<ResultTree> fullOutputsQueueTemp = new LinkedList<ResultTree>(fullOutputsQueue);
						Visualizer.visualizeOutput(fullOutputsQueueTemp, graph, null, newKeywords);
					}
					...
					System.out.println("after new keyword search answers: " + discoveredAnswersNewKWS);
				...
				if (coOccBaseLineOn) {
					...
					topNAnswersTemp = tansformResultTreeIntoAnswerAsInput(this.fullOutputsQueue);
					...
					if (coOccHandler.topFrequentKeywords.size() > 0) {
						...
						if (qualityExp) {
							System.out.println(
									"selected keyword: " + coOccHandler.topFrequentKeywords.get(0) + " num of cand: "
				...
				if (dataCloudOn) {
					...
					if (dataCloudHandler.topFrequentKeywords.size() > 0) {
						...
						if (qualityExp) {
							System.out
									.println("selected keyword: " + dataCloudHandler.topFrequentKeywords.get(0)
											+ " num of cand: "
											+ nodeIdsOfToken.get(StringPoolUtility
			...
			timeInfos.add(
					new InfoHolder(14, "qExp Vis. Nodes", rootKWSExpand != null ? rootKWSExpand.visitedNodes : ""));
			timeInfos.add(
					new InfoHolder(15, "qExp Vis. Kywrds", rootKWSExpand != null ? rootKWSExpand.visitedKeywords : ""));
			timeInfos.add(new InfoHolder(16, "qExp getKywrds Time",
					rootKWSExpand != null ? rootKWSExpand.getKeywordsDuration : ""));
			timeInfos.add(new InfoHolder(17, "sugg kywrds (freq pruned)",
					rootKWSExpand != null ? rootKWSExpand.estimatedWeightOfSuggestedKeywordMap.size() : ""));
			...
			timeInfos.add(new InfoHolder(23, "incEval Duration", incEval != null ? incEval.incEvalDuration : ""));
			timeInfos.add(new InfoHolder(24, "incEval visited nodes", incEval != null ? incEval.visitedNodes : ""));
			timeInfos.add(new InfoHolder(25, "incEval total cost", incEvalOn ? incEvalTotalCostPlusInitCost : ""));
			timeInfos.add(new InfoHolder(26, "new KWS time", newKWSOn ? secondaryKWSDuration : ""));
			timeInfos.add(new InfoHolder(27, "new KWS quality", newKWSOn ? secondaryKWSQuality : ""));
			timeInfos.add(new InfoHolder(28, "discoveredAnswersNewKWS", newKWSOn ? discoveredAnswersNewKWS : ""));
			timeInfos.add(new InfoHolder(29, "avg out deg cand2", newKWSOn ? avgOutDegreeOfKeywordCandidates : ""));

			timeInfos.add(new InfoHolder(30, "KWS time for coOcc", coOccBaseLineOn ? coOccKWSDuration : ""));
			timeInfos.add(new InfoHolder(31, "KWS quality for coOcc", coOccBaseLineOn ? coOccKWSQuality : ""));
			timeInfos
					.add(new InfoHolder(32, "discoveredAnswers co occ", coOccBaseLineOn ? discoveredAnswersCoOcc : ""));
			timeInfos.add(
					new InfoHolder(33, "coOccHandler", (coOccBaseLineOn && coOccHandler.topFrequentKeywords.size() > 0)
							? coOccHandler.topFrequentKeywords.get(0) : ""));

			timeInfos.add(new InfoHolder(34, "KWS time for dataCloud", dataCloudOn ? dataCloudKWSDuration : ""));
			timeInfos.add(new InfoHolder(35, "KWS quality for dataCloud", dataCloudOn ? dataCloudKWSQuality : ""));
			timeInfos.add(
					new InfoHolder(36, "discoveredAnswers dataCloud", dataCloudOn ? discoveredAnswersDataCloud : ""));
			...
			timeInfos.add(new InfoHolder(38, "minSuggestedWeightMainDelta", minSuggestedWeightMainDelta));
			timeInfos.add(new InfoHolder(39, "avgSuggestedWeightMainDelta", avgSuggestedWeightMainDelta));
			timeInfos.add(new InfoHolder(40, "maxSuggestedWeightMainDelta", maxSuggestedWeightMainDelta));

			timeInfos.add(new InfoHolder(41, "minSuggestedWeightSecondDelta", minSuggestedWeightSecondDelta));
			timeInfos.add(new InfoHolder(42, "avgSuggestedWeightSecondDelta", avgSuggestedWeightSecondDelta));
			timeInfos.add(new InfoHolder(43, "maxSuggestedWeightSecondDelta", maxSuggestedWeightSecondDelta));
...
//			continue;
		...
		return fullOutputsQueue;
	}

	public void preKWS(ArrayList<String> keywords, int exp) throws Exception {
	...
	public LinkedList<ResultTree> runFromOutside(ArrayList<String> keywords, HashMap<String, HashSet<Integer>> candidatesOfAKeyword,
	...
	private boolean run(ArrayList<String> keywords) throws Exception {
		...
		for (String keyword : keywords) { // keywords is an ArrayList - enhanced
			...
			for (int sourceNodeId : candidatesOfAKeyword.get(keyword)) {
				...
				//debug
				NodeInfra node = graph.nodeOfNodeId.get(sourceNodeId);
				System.out.println("Node " + node.nodeId);
				for (int tokenId : node.tokens) {
					System.out.println("Token " + tokenId + " = " + StringPoolUtility.getStringOfId(tokenId));
				}
			...
			if (nextExtendedNode.originsOfKeywords.keySet().size() == keywords.size()) {
				...
				for (HashMap<String, HashSet<Integer>> keywordNodeIdsParticipatingInTree : crossedProductResults) {
					...
					if (outputHeap.size() == heapSize) {
						...
						//debug
						//fullOutputsQueue.add(resultTreeToBeRemovedAndPrinted);
		...
		while (!outputHeap.isEmpty() && totalPrintedResults < maxRequiredResult) {
			...
			//
			// content.add(new Demo(resultTreeToBeRemovedAndPrinted));
			// frame.pack();
	...
	public void print(ResultTree resultTreeToBePrinted) throws Exception {
		...
		if(hasSameRootWithPreviousOnes(fullOutputsQueue, resultTreeToBePrinted)){
			return;
		}
	...
	private boolean hasSameRootWithPreviousOnes(LinkedList<ResultTree> fullOutputsQueue, ResultTree resultTreeToBePrinted) {


		for(ResultTree oldResTree : fullOutputsQueue){
			if(oldResTree.rootNode.nodeId == resultTreeToBePrinted.rootNode.nodeId){
				return true;
			}
		}
		return false;
	...
	public ArrayList<AnswerAsInput> tansformResultTreeIntoAnswerAsInput(LinkedList<ResultTree> fullOutputsQueue) {
		ArrayList<AnswerAsInput> topNAnswers = new ArrayList<AnswerAsInput>();
	...
	public HashMap<Integer, CostAndNodesOfAnswersPair> keywordExpand(ArrayList<AnswerAsInput> topNAnswers,
			ArrayList<String> keywords, double cost, int exploreRange) throws Exception {
		...
		rootKWSExpand = new RootKWSExpand(graph, topNAnswers, cost, exploreRange, keywordsSet);
	...
	public void generateBipartiteGraph(ArrayList<AnswerAsInput> topNAnswers, ArrayList<String> keywords)
			throws Exception {
		...
		rootKWSExpand.expandForBiClique();
		HashMap<Integer, HashMap<Integer, Double>> answerKeywordMap = rootKWSExpand.answerKeywordMap;

		HashMap<Integer, NodeInfra> visitedNode = new HashMap<Integer, NodeInfra>();

		// BipartiteGraph, U -> keyword, V -> top answers
		BipartiteGraph bipartiteGraph = new BipartiteGraph();
		for (int topAnswer : answerKeywordMap.keySet()) {
			// V side node is top-n answer, weight is 1
			// U side node is keyword node, weight is distance to root
			bipartiteGraph.getAndAddANewBipartiteNodeInfra(topAnswer, 1, BipartiteSide.VSide);
			HashMap<Integer, Double> keywordDistance = answerKeywordMap.get(topAnswer);

			for (int keyword : keywordDistance.keySet()) {
				if (visitedNode.containsKey(keyword)) {
					NodeInfra node = visitedNode.get(keyword);

					float weight = (float) keywordDistance.get(keyword).doubleValue();
					node.weight = weight;

					// add relationship
					bipartiteGraph.addBipartiteRelationship(topAnswer, node.nodeId, weight);
					bipartiteGraph.addBipartiteRelationship(node.nodeId, topAnswer, weight);
				} else {
					float weight = (float) keywordDistance.get(keyword).doubleValue();
					NodeInfra node = bipartiteGraph.getAndAddANewBipartiteNodeInfra(keyword, weight,
							BipartiteSide.USide);
					visitedNode.put(keyword, node);
